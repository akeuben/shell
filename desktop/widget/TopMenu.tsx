...
class LineGraphWidget extends Gtk.DrawingArea {
    ...
    vfunc_snapshot(snapshot: Gtk.Snapshot) {
        ...
        for (const xp of xs) {
            ...
            verticalPath.line_to(x + this.padding, height - this.padding);
            const label = `${xp}`;
            layout.set_text(label, label.length); // label for this line
            const point = new Graphene.Point({x: x + this.padding - layout.get_pixel_size()[0]/2, y: height - this.padding});
        ...
        this.points.points.forEach((p, i) => {
            ...
            const y = height - (p.y - minY) * scaleY - this.padding; // invert y-axis
            if(i == 0) {
                x -= 2;
                firstX = x;
            }
        ...
        path.line_to(lastX, height - this.padding); // bottom right
        path.line_to(firstX, height - this.padding); // bottom right
        path.close();
...
const PCBattery = ({battery}: {battery: AstalBattery.Device}) => {
    ...
    const history = createExternal(new PointsList([]), (set) => {
        const update = () => {
            const history = getStatistics(`/org/freedesktop/UPower/devices/battery_${battery.native_path}`);
